...
SECTION_RE = re.compile(r"^\s*(\d+)\s*\.\s*(.+?)\s*$")  # 例如: "1.Right arm（右手臂動）"
CSV_RE = re.compile(r"^\s*-?\d+(?:\s*,\s*-?\d+)*\s*$")  # 逗號分隔整數

def parse_move_file(path: Path) -> Dict[Tuple[int, str], List[str]]:
    """
    將 move.txt 解析成：
    { (section_number, section_title) : [ "0,0,...", "..." , ... ] }
    允許各段之間有空行。
    只收集「逗號分隔整數」的行，其他行忽略。
    """
    if not path.exists():
    ...
    with path.open("r", encoding="utf-8") as f:
        for raw in f:
            ...
            if CSV_RE.match(line):
                if cur_key is None:
                    # 未遇到標題也有 CSV 行：歸到 0 段
                    cur_key = (0, "Default")
                    sections.setdefault(cur_key, [])
                sections[cur_key].append(",".join(x.strip() for x in line.split(",")))
            # 其他情形忽略（例如說明文字）
    # 驗證每行長度一致（17值），若不一致就報警告（不擋）
    for key, frames in sections.items():
        for i, frame in enumerate(frames):
            cols = [c.strip() for c in frame.split(",")]
            if len(cols) != 17:
                print(f"[warn] 第 {key[0]} 段「{key[1]}」第 {i+1} 行欄位數非 17（是 {len(cols)}）：{frame}")
    if not sections:
...
def main():
    ap = argparse.ArgumentParser(description="餵角度：當 angles.txt 為空時，從 move.txt 寫入下一筆。")
